...
class RegisterSerializerWithToken(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if not data.get('name'):
            raise serializers.ValidationError({"Error":"Name can not be empty"})
        if not data.get('email'):
            raise serializers.ValidationError({"Error":"Email can not be empty"})
        if not data.get('password'):
            raise serializers.ValidationError({"Error":"Password can not be empty"})
        return data
